...
Webapp for tutors, promotors and operators
...
- run: `./bump-version.sh <release> release`
- commit
- run: `mvnw -Pprod clean install jib:build -DskipTests -Dmaven.test.failure.ignore=true`
- finish release (gitflow)
- on develop branch, ensure the correct (next) dev version, running: `./bump-version.sh <next dev version> dev`
- commit & push

# famility_takeme_backoffice
...
To build the final jar and optimize the famility_backoffice application for production, run:
